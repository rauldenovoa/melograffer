...
| 7 | Website/app UI design revamp (BACKLOG #26, 27) | The app's *container* — page layout, typography, color palette, panels, buttons, header/chrome — redesigned to read as a professional, elegant, understated product (not flashy); distinct from M13, which is the canvas visualization's own rendering styles, not the page around it | opusplan |
| 8 | Housekeeping & quick UI wins (BACKLOG #20, 21, 1, 2) | Fixture licensing resolved (kept or replaced with confirmed-clear provenance); About page live crediting Malinowski/MAM; `F` toggles fullscreen; Tracks panel scrolls past ~3 tracks instead of growing unbounded | opusplan |
| 9 | Export UX polish (BACKLOG #25, 24) | Root cause of the Export→Save-As delay identified and mitigated (or documented as irreducible); a pre-export preview is available before committing to a full encode+download | opusplan |
| 10 | Voice separation & chord mapping (BACKLOG #3, 6) | Single-track MIDI (e.g. the Arabesque fixture) auto-splits into distinct colored voices via a documented heuristic; tracks with simultaneous notes per voice (chords) render correctly instead of assuming one sounding note at a time | opusplan |
| 11 | Sounding-note animation polish (BACKLOG #7, 8, 9) | Clone can optionally track the melody's pitch instead of a fixed position; outline-flash-brighter option available as an alternative to today's decay; "organic" nonlinear scroll easing around the sounding note is toggleable — each judged visually | opusplan |
| 12 | Audio quality (BACKLOG #22, 5, 10, 15) | GarageBand-vs-synth quality gap investigated and documented with concrete causes; based on findings, ship whichever of per-track GM instruments / swappable soundfont / SF2 ADSR envelope are judged worthwhile | opusplan |
| 13 | Visualization styles & design revamp (BACKLOG #23, 4, 13, 12, 14) | MAMPlayer comparison notes written; visual design revamp shipped; modular renderer architecture supports the existing dot style plus bar/ribbon shapes and a harmonic-coloring option, user-selectable | opusplan |
| 14 | Presets & library (BACKLOG #11) | Users can save/load named presets (config + theme) and browse a small piece library without re-uploading files each time | opusplan |
